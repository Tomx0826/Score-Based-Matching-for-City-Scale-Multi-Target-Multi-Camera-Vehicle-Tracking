...
def find_ref_and_border_frames(tracklets):
    ...
    for cam_id in tracklets.keys():
        ...
        for obj_id in tracklets[cam_id].keys():
            ...
            tracklet = tracklets[cam_id][obj_id] # list of [frame_id, x1, y1, w, h]
    ...
    for cam_id in wh_ratio_dict.keys():
        ...
        for obj_id in wh_ratio_dict[cam_id].keys():
            if cam_id in both_two_border.keys() and \
               ...
               only_one_border: #only_one_border=False
                continue
            if 'border' in wh_ratio_dict[cam_id][obj_id]:
                ...
                if 'small_ref' in wh_ratio_dict[cam_id][obj_id]:
                    small_ref = wh_ratio_dict[cam_id][obj_id]['small_ref'] #frame_id
                else:
                    small_ref = 9999999999999
                if 'big_ref' in wh_ratio_dict[cam_id][obj_id]:
                    big_ref = wh_ratio_dict[cam_id][obj_id]['big_ref'] # frame_id
                else:
                ...
                if big_ref < 2200:
                    num = 0
                    for i in range(big_ref - mean_f, big_ref): #靠近Border
                ...
                if small_ref < 2200:
                    num = 0
                    for i in range(small_ref, small_ref + mean_f): #遠離Border
                        if i not in wh_ratio_dict[cam_id][obj_id]['normal'].keys(): 
                            continue
                        num += 1
                ...
                for border_i in border_fid:
                    if abs(border_i - big_ref) < abs(border_i - small_ref) and big_ref < 2200: # border frame 靠近 big_ref
                        ...
                        if (wh_ratio > 1.5 or wh_ratio < 0.5) or (area_ratio < 0.7 or area_ratio > 1.3):
                            ...
                            remove_num += 1
                    if abs(border_i - big_ref) > abs(border_i - small_ref) and small_ref < 2200: # border frame 靠近 small_ref
...
remove_bboxes, remove_num = remove_border_boxes(wh_ratio_dict, both_two_border, only_one_border=False) #在border被截掉的Box
