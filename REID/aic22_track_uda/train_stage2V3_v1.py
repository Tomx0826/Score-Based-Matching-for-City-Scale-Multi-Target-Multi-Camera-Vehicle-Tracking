...
from model.make_model4 import make_model
#from model.make_model5 import make_model
from solver import make_optimizer, WarmupMultiStepLR
from loss.make_loss_ori import make_loss
...
def cal_class_weight(pids):
    dict_weight = defaultdict(zero)
    for pid in pids:
        dict_weight[pid] += 1
    class_weights = []
    for i in range(len(set(pids))):
        dict_weight[i] = dict_weight[i]
    for pid in sorted(dict_weight.keys()):
        class_weights.append(dict_weight[pid])
...
if __name__ == '__main__':
    ...
    for epoch in range(cfg.SOLVER.MAX_EPOCHS):
        ...
        if epoch % 81 == 0: # and epoch < 9) or (epoch % 6 == 0):
            print('Extract Feature Starting...')
            target_features, _, target_camids, target_trkids = extract_features(model, 
                                                                                val_loader, 
                                                                                print_freq=100)
            ...
            P, neg_vec = compute_P2(target_features, target_features, 
                                    target_camids, la=cfg.STAGE2.LA)
            ...
            for i, trkid in enumerate(unique_trkids):
                track_feature = torch.mean(gallery_features[gallery_trkids == trkid], 
                                           dim=0, keepdim=True)
                tmp_indices = (gallery_trkids == trkid)
                gallery_features[tmp_indices] = gallery_features[tmp_indices]*0.3 + \
                                                track_feature*0.7
            ...
            print('new class are {}, length of new dataset is {}'.format( \
                  len(set(pids)), len(new_dataset)))
            class_weights = cal_class_weight(pids)
        
        train_loader = IterLoader(get_trainloader_uda(cfg, new_dataset, num_classes))
        train_loader.new_epoch() 
        loss_func, center_criterion = make_loss(cfg, num_classes=num_classes, class_weights=class_weights) #, class_weights=class_weights
