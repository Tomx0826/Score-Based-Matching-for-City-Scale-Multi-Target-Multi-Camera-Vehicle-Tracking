    ...
    for n_iter in range(3985):
        ...
        if cfg.SOLVER.FP16_ENABLED:
            with amp.autocast(enabled=True):
                #score, feat, imClip,imClip1,imClip2 = model(img, target, cam_label=target_cam)
                score, feat = model(img, target, cam_label=target_cam)
                #loss = loss_fn(score,feat,img,imClip,imClip1,imClip2,target)
        ...
        if (n_iter + 1) % log_period == 0:
            logger.info('Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Acc: {:.3f}'.format(epoch, (n_iter+1), 3985, loss_meter.avg, acc_meter.avg))
