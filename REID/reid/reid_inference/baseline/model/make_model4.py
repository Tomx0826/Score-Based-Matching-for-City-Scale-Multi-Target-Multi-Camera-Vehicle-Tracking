...
from .layers.image_reconst import MSDecoder
...
class Backbone(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        self.noise_factor = cfg.MS_RECONST.NOISE_FACTOR
        ...
        self.decoder_head = MSDecoder(feat_dim)
    ...
    def forward(self, x, label=None, cam_label=None):  
        ...
        if self.model_name =='efficientnet_b7':
            ...
            #x, x_layer3 = self.base(x)
            x = self.base(x)
        #ul4x_layer_size = x.layer3.shape[1:4]
        ...
        if self.neck == 'no':
            ...
            #feat_norm =  torch.sqrt(torch.sum(torch.square(feat), dim=1))
        ######################
        if self.training:
            #imClip,imClip1,imClip2 = self.decoder_head(feat)
            cls_score = self.classifier(feat) 
            return cls_score, feat #, imClip,imClip1,imClip2
        else:
            if self.neck_feat == 'after':
                #  print("Test with feature after BN")
                #feat_norm = torch.sqrt(torch.sum(torch.square(feat), dim=1))
                #feat = torch.div(feat.t(),feat_norm)
                #feat = feat.t()
                return feat
            else:
                #  print("Test with feature before BN")
                #feat_norm = torch.sqrt(torch.sum(torch.square(global_feat), dim=1))
                #global_feat = torch.div(global_feat.t(),feat_norm)
