...
class KalmanFilter(object):
    ...
    def initiate(self, measurement):
        ...
        #利用高度(height)表示物體遠近，遠近程度影響移動距離的預測
        std = [
    ...
    def predict(self, mean, covariance):
        ...
        # x' = Fx -- function 1 / F=_motion_mat
        mean = np.dot(self._motion_mat, mean)
        
        # P' = FPF^T+Q -- function 2
    ...
    def project(self, mean, covariance, confidence=.0):
        ...
        # NSA kalman filter / noise vs confidence
        if opt.NSA: # True
            std = [(1 - confidence) * x for x in std]
        
        # noise R
        innovation_cov = np.diag(np.square(std)) # Noise, 對角矩陣, 4x4, 

        # Hx' / H=self._update_mat (4*8)
