...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=30, n_init=3, high_score=0.6, cam_name=None, mask=None, image_filenames=None):
        ...
        self.match_b_thresh = 0.4
        if cam_name == 'c041':
            self.high_score = 0.5
        #if cam_name == 'c042':
        #    #self.high_score = 0.6
        #    self.metric.matching_threshold = 0.5
        #    self.match_b_thresh = 0.5
        #    self.max_iou_distance = 0.75
        #    pass
    ...
    def update(self, detections):
        ...
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(detections[detection_idx]) #更新smooth feature, smooth_color_hist, self.hits += 1, self.time_since_update = 0
        #條件track_n_init < 3 or miss_age > 30 ==> deleted
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices, enable_motion_shape=False, color_gate=True):
            ...
            # cost_matrix:
            # T個track, L個det做計算
            # step 1. 假如第一個track有N個sample, feature_dim = 2048, N個sample(Nx2048)跟L個dets做Cosine similarity (NxL)
            # step 2. 1 - similarity matrix得到distance matrix(NxL)
            # step 3. 取distance matrix各個column最小值(1xL) 
            #         --> 即表示從L個det boxes中，逐個針對每一個det box,從track中取一張跟該det box最相近的一張sample做距離計算
            # step 4. cost matrix(TxL)的第一個row(第一個track)即為步驟3的結果
            ...
            # c043 c044 --> gating_threshold=9.4877, add_identity=False
            # others --> gating_threshold=50.0 add_identity=True
            
            # 用馬氏距離作為mask屏蔽Cosine distance cost matrix中的值
        ...
        match_b_thresh = self.match_b_thresh
        ...
        self.draw_kf_pred(self.tracks, detections, matches, unmatched_tracks, unmatched_detections)
        
        return matches, unmatched_tracks, unmatched_detections
    
    def draw_kf_pred(self, tracks, detections, matches, unmatched_tracks, unmatched_detections):
        cam = self.cam_name
        if len(detections) == 0:
            return
        cur_frame = "%04d" % detections[0].frame_idx

        img_dir = f"/Disk_New/AI_City_Challenge/AICITY2022-Track1-MTMC/SCMT_2/dataset/CityFlowV2/AICITY/test/{cam}/img1/"
        img_path = img_dir + f'{cur_frame}' + '.jpg'

        img = cv2.imread(img_path)
        max_height, max_width, _ = img.shape

        #out_path = f"/Disk_New/AI_City_Challenge/AICITY2022-Track1-MTMC/SCMT/kf_viz/{cam}/"
        #out_path = f"/Disk_New/AI_City_Challenge/AICITY2022-Track1-MTMC/SCMT_2/unmatches_viz_10fps/track/{cam}/"
        out_path =  f"/Disk_New/AI_City_Challenge/AICITY2022-Track1-MTMC/SCMT_2/det_viz_10fps_id_284/track/{cam}/"
        ...
        for d in detections:
            ...
            img  = cv2.rectangle(img, pt1=(x,y), pt2=(x2,y2), color=color, thickness=1)
            if confidence<0.6:
                cv2.putText(img, "c={:.2f}".format(confidence), (x+1,y+1), cv2.FONT_HERSHEY_DUPLEX, 1, color, 1, cv2.LINE_AA)  
        ...
        #for t, d in matches:
        #    color = (255,0,0)
        #    x, y, w, h = tracks[t].to_tlwh()
        #    tid = tracks[t].track_id
        #    if tid!= 75:
        #        continue
        #    x2 = int(x+w)
        #    y2 = int(y+h)
        ...
        #    
        #    img  = cv2.rectangle(img, pt1=(x,y), pt2=(x2,y2), color=color, thickness=1)
        #    cv2.putText(img, f"{tid}", (x+2,y+1), cv2.FONT_HERSHEY_DUPLEX, 1, color, 1, cv2.LINE_AA)
        #     
        #    #x, y, w, h = detections[d].tlwh
        ...
        #    #
        #    #img  = cv2.rectangle(img, pt1=(x,y), pt2=(x2,y2), color=color, thickness=1)
        #    ##cv2.putText(img, "   c={:.2f}".format(confidence), (x+2,y+1), cv2.FONT_HERSHEY_DUPLEX, 1, color, 1, cv2.LINE_AA)           
        #
        #
        
        #for ut in unmatched_tracks:
        #    color = (0, 255, 0)
        #    x, y, w, h = tracks[ut].to_tlwh()
        #    tid = tracks[ut].track_id
        #    #if tid!= 75:
        #    #    continue
        #    x2 = int(x+w)
        #    y2 = int(y+h)
        ...
        #    color = (0,0,255)
        #    x, y, w, h = detections[ud].tlwh
        #    confidence = detections[ud].confidence
        #    
        #    x2 = int(x+w)
        #    y2 = int(y+h)
        #    x = int(x)
        ...
        #    img  = cv2.rectangle(img, pt1=(x,y), pt2=(x2,y2), color=color, thickness=1)
        #    #cv2.putText(img, "c={:.2f}".format(confidence), (x+2,y+1), cv2.FONT_HERSHEY_DUPLEX, 1, color, 1, cv2.LINE_AA)      
        cv2.imwrite(out_path+ f'{cur_frame}' + '.jpg', img)
        ...
        for track in self.tracks_all:
            ...
            for det in track.storage:
                ...
                if f_pre + 1 < f_curr < f_pre + interval:
                    # f_pre跟f_curr差距為2~20之間(非下一張但又不會差太遠)
